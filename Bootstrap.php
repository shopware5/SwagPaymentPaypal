...
EOD;
    ...
    /**
     * Return path to paypal logo generated by media manager
     *
     * @return string
     */
    public function getLogoPath()
    {
        ...
        $image = $mediaRepo->findOneBy(array('name' => $fileName));
        if ($image) {
            $logoPath = $image->getPath();
            if ($this->isShopware51()) {
                $mediaService = $this->get('shopware_media.media_service');
                $logoImage = $mediaService->getUrl($logoPath);
                if ($logoImage) {
                    return $logoImage;
                }
            } else {
    ...
    /**
     * Return information for paypal logo file
     *
     * @return array
     */
    private function getMediaLogo()
    {
        //location to paypal logo
        $path = __DIR__ . '/Views/frontend/_resources/images/paypal_logo.png';
